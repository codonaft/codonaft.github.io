...
## Полезные ссылки
- видео с конференций
    - [Pacific++]({{ site.theme_settings.foss_frontends.youtube }}/channel/UCrRR5mU5aqvtZAuEGYfdTjw/videos?flow=grid&sort=p&view=0)
    - [CppCon]({{ site.theme_settings.foss_frontends.youtube }}/user/CppCon/videos)
    - [C++ Russia]({{ site.theme_settings.foss_frontends.youtube }}/channel/UCJ9v015sPgEi0jJXe_zanjA/videos?sort=p&view=0&flow=grid)
    - *(список дополняется)*
- [Статические анализаторы]({{ site.theme_settings.foss_frontends.github }}/analysis-tools-dev/static-analysis#cpp)
...
- [Путеводитель C++ программиста по неопределенному поведению]({{ site.theme_settings.foss_frontends.github }}/Nekrolm/ubbook#readme)
...
[актуального стандарта C++]({{ site.theme_settings.foss_frontends.wikipedia_en }}/wiki/C%2B%2B#Standardization).
...
Для `g++` и `clang++` это [не очень очевидно делается]({{ site.theme_settings.foss_frontends.stackoverflow }}/questions/11714827/how-to-turn-on-literally-all-of-gccs-warnings).
...
просмотром [YouTube]({{ site.theme_settings.foss_frontends.youtube }}/results?sp=EgIQAw%253D%253D&search_query=gentoo+install)
и поиском в Google ;)
